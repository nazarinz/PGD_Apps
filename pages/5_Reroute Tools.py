...
class Config:
    ...
    NEW_PO_RENAMES = {
        ...
        'MTFC Number': 'Customer Contract ID',
    }

    OLD_PO_RENAMES = {'Article Number': 'Cust Article No.'}

    HEADER_TOKENS = [
        'articlenumber','articleno','article','modelname','ponumber',
        'marketponumber','customerrequestdate','plandate','grandtotal',
        'classcode','gpscustomernumber','shipmentmode','plantcode',
        'orderquantity','customercontractid','custarticleno',
    ]

    DATE_TOKENS = [
        'po date','crd','pd','fpd','posdd','lpd','podd',
        'vas cut-off date','document date',
    ]
...
class SizeColumnHandler:
    ...
    def canonical_size_name(col: str) -> str:
        ...
        if m:
            return f'UK_{m.group(1)}{m.group(2) or ""}'
    ...
    def normalize_article_column(df):
        ...
        if 'Cust Article No.' not in df.columns:
            ColumnManager.alias_column(df, 'Cust Article No.',
                ['Cust Article No.', 'Article Number', 'Article No', 'Article'],
                ['article', 'number'])
        if 'Cust Article No.' not in df.columns:
...
class CustomerContractNormalizer:
    ...
    def propagate_across_groups(df):
        if 'Customer Contract ID' not in df.columns:
            return df
        df = df.copy()
        ...
        if 'Sold-To PO No.' in df.columns:
            group_keys.append('Sold-To PO No.')
...
class POPreparation:
    ...
    def prepare(df, remark, sort_order, include_sales_order=False):
        ...
        if 'Order Quantity' not in df.columns:
            df['Order Quantity'] = pd.NA
        if 'Customer Contract ID' not in df.columns:
            df['Customer Contract ID'] = pd.NA
...
class SizeComparison:
    ...
    def create_comparison(merged_df, size_cols):
        ...
        if 'Sold-To PO No.' in merged_df.columns:
            cols_needed.append('Sold-To PO No.')
        ...
        base['comparison_key'] = (base['Cust Article No.'].astype(str) + '|' +
                                  base['OrderQty_fix'].astype(str) + '|' +
                                  base['size_sig'].astype(str))
...
def get_date_columns(df):
    ...
    for col in df.columns:
        cl = col.lower()
        if 'drc' in cl or '-dr' in cl:
            continue
        if any(t in cl for t in Config.DATE_TOKENS):
    ...
    with pd.ExcelWriter(buf, engine='xlsxwriter', datetime_format='m/d/yyyy') as writer:
        ...
        for col in Config.TEXT_FORMAT_COLUMNS:
            if col in main_df.columns:
                idx = main_df.columns.get_loc(col)
                ws.set_column(idx, idx, 18, text_fmt)
        ...
        if not compare_df.empty:
            compare_df.to_excel(writer, sheet_name='Size_Compare', index=False)
        if not only_diff.empty:
            only_diff.to_excel(writer, sheet_name='Only_Different', index=False)
        if not only_equal.empty:
            only_equal.to_excel(writer, sheet_name='Only_Equal', index=False)
...
EMAIL_PREP_TARGET = [
    ...
    "Email Subject","Prod. Status","Remark2",
]
...
def page_home():
    ...
    with col1:
        ...
        <div class="tool-card">
            <h3>🔀 Reroute Merge</h3>
            <p>Gabungkan Old PO (Canceled) dan New PO (Reroute) dalam satu file dengan breakdown size, validasi, dan color-coding otomatis.</p>
        </div>""", unsafe_allow_html=True)
    with col2:
        st.markdown("""
        <div class="tool-card">
            <h3>📋 Input Pending Cancel</h3>
            <p>Filter baris Old PO dari file merge, lalu ekspor ke 2 sheet: <em>Full</em> (lengkap) dan <em>Summary</em> (ringkasan).</p>
        </div>""", unsafe_allow_html=True)
    with col3:
        st.markdown("""
        <div class="tool-card">
            <h3>✉️ Input Tracking Reroute Report</h3>
            <p>Reorder kolom, isi Email Subject dan Remark2 otomatis, siap kirim ke customer.</p>
        </div>""", unsafe_allow_html=True)
...
def page_reroute_merge():
    ...
    with col_new:
        file_new = st.file_uploader("📄 New PO (Reroute)", type=["xlsx","xls"], key="merge_new")
    with col_old:
        file_old = st.file_uploader("📄 Old PO (Canceled)", type=["xlsx","xls"], key="merge_old")

    st.markdown('<hr class="sect-divider">', unsafe_allow_html=True)

    if not (file_new and file_old):
        st.info("Upload kedua file untuk melanjutkan.")
        return

    step(2, "Konfigurasi")
    col_a, col_b = st.columns(2)
    with col_a:
        include_sales = st.checkbox("Sertakan Sales Order dalam merge key", value=False)
    with col_b:
        output_name = st.text_input("Nama file output", value="Merged_Old_New_V2.xlsx")

    st.markdown('<hr class="sect-divider">', unsafe_allow_html=True)

    step(3, "Proses")
    if not st.button("▶ Jalankan Merge"):
        return
    ...
    with st.spinner("Memproses…"):
        try:
            ...
            for c in date_cols:
                final[c] = pd.to_datetime(final[c], errors='coerce')
            ...
            st.markdown(stat_box_html({
                ...
                "Old PO": n_old,
                "New PO": n_new,
                "Size Cols": len(used_sz),
                "Diff Articles": len(only_diff) if not only_diff.empty else 0,
            }), unsafe_allow_html=True)
            ...
            st.download_button(
                label="⬇️ Download Hasil Merge (.xlsx)",
                data=excel_bytes,
...
def page_old_po_filter():
    ...
    col_a, col_b = st.columns(2)
    with col_a:
        out_name = st.text_input("Nama file output", value="Old_PO_Filtered.xlsx")

    if not uploaded:
        st.info("Upload file untuk melanjutkan.")
        return

    st.markdown('<hr class="sect-divider">', unsafe_allow_html=True)
    step(3, "Proses")
    if not st.button("▶ Jalankan Filter Old PO"):
        return
    ...
    with st.spinner("Memproses…"):
        try:
            ...
            if not all_filtered:
                st.warning("Tidak ada baris 'Old PO' ditemukan.")
                return
...
def page_email_prep():
    ...
    <div class="app-header">
        ...
        <span class="badge">Auto-fill · Multi-sheet</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    col_a, col_b = st.columns(2)
    with col_a:
        email_subject = st.text_input("Email Subject (berlaku semua baris)", placeholder="Contoh: Reroute PO - May 2025")
    with col_b:
        out_name = st.text_input("Nama file output", value="Email_Prep.xlsx")

    st.markdown("""
    <div style="background:#0f1117;border:1px solid #1e2a3a;border-radius:8px;padding:12px 16px;margin:8px 0;">
        <span style="font-family:'IBM Plex Mono',monospace;font-size:0.78rem;color:#60a5fa;">Kolom output yang dihasilkan:</span><br>
        <span style="font-size:0.78rem;color:#94a3b8;">Sales Order · Remark · Sold-To PO No. · Ship-To Party PO No. · Model Name · Cust Article No. · Article · Order Quantity · CRD · PD · LPD · PODD · Ship-To Search Term · Ship-To Country · <strong style="color:#60a5fa;">Email Subject</strong> · Prod. Status · <strong style="color:#60a5fa;">Remark2 (= "Full Reroute")</strong></span>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not email_subject.strip():
        st.warning("Isi Email Subject terlebih dahulu.")
        return

    st.markdown('<hr class="sect-divider">', unsafe_allow_html=True)
    step(3, "Proses")
    if not st.button("▶ Jalankan Email Prep"):
        return
    ...
    with st.spinner("Memproses…"):
        try:
            ...
            for sname, df in xls.items():
                ...
                missing = [c for c in EMAIL_PREP_TARGET if c not in df.columns and c not in ("Email Subject","Remark2","Prod. Status")]
            ...
            st.markdown(stat_box_html({
                ...
                "Sheet": len(sheets_out),
                "Kolom Output": len(EMAIL_PREP_TARGET),
            }), unsafe_allow_html=True)
            ...
            st.download_button(
                label="⬇️ Download Email Prep (.xlsx)",
                data=excel_bytes,
                file_name=out_name,
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
