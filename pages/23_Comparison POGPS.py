...
# ================== [NEW] FALSE Insight — OPEN Orders ==================
def render_false_insight_open(df_view: pd.DataFrame):
    """
    Menampilkan panel insight: jumlah FALSE untuk semua kolom Result_*,
    difilter hanya Order Status Infor == OPEN.
    """
    ALL_RESULT_COLS = [
        "Result_Quantity",
        "Result_Model Name",
        "Result_Article No",
        "Result_Classification Code",
        "Result_Lead Time",
        "Result_Sort1",
        "Result_Country",
    ...
    # Label yang lebih ramah buat tampilan
    LABEL_MAP = {
    ...
    st.markdown("---")
    st.subheader("🔴 Insight FALSE — Order Status: OPEN")
    ...
    # Hitung TRUE / FALSE / blank hanya untuk kolom yang ada
    existing_result_cols = [c for c in ALL_RESULT_COLS if c in df_open.columns]
    if not existing_result_cols:
        st.info("Tidak ada kolom Result_* yang tersedia.")
        return

    rows = []
    for col in existing_result_cols:
        label      = LABEL_MAP.get(col, col)
        true_cnt   = int(df_open[col].eq("TRUE").sum())
        false_cnt  = int(df_open[col].eq("FALSE").sum())
        blank_cnt  = int((~df_open[col].isin(["TRUE", "FALSE"])).sum())
        evaluated  = true_cnt + false_cnt          # hanya yang dievaluasi (exclude blank)
        false_pct  = round(false_cnt / evaluated * 100, 1) if evaluated > 0 else 0.0
        true_pct   = round(true_cnt  / evaluated * 100, 1) if evaluated > 0 else 0.0
        rows.append({
            "Field":         label,
            "Result Column": col,
            "TRUE":          true_cnt,
    ...
    # ── KPI Ringkas ─────────────────────────────────────────────────────
    total_false_all = insight_df["FALSE"].sum()
    worst_field     = insight_df.iloc[0]["Field"] if not insight_df.empty else "-"
    worst_false_pct = insight_df.iloc[0]["FALSE %"] if not insight_df.empty else 0

    kpi1, kpi2, kpi3, kpi4 = st.columns(4)
    kpi1.metric("Total PO (OPEN)", f"{total_open:,}")
    kpi2.metric("Total FALSE (semua field)", f"{total_false_all:,}")
    kpi3.metric("Field paling banyak FALSE", worst_field)
    kpi4.metric("FALSE % tertinggi", f"{worst_false_pct}%")

    st.markdown("#### 📋 Detail FALSE per Field (OPEN Orders)")

    # Tabel dengan highlight FALSE % tinggi
    def highlight_false(row):
        pct = row["FALSE %"]
        if pct >= 50:
            color = "background-color: #FFCCCC"   # merah muda
        elif pct >= 20:
            color = "background-color: #FFE5B4"   # oranye muda
        elif pct > 0:
            color = "background-color: #FFFACD"   # kuning muda
        else:
            color = ""
        return [color] * len(row)

    display_cols = ["Field", "TRUE", "FALSE", "FALSE %", "TRUE %", "Evaluated", "N/A (blank)"]
    styled_df    = insight_df[display_cols].style.apply(highlight_false, axis=1)
    st.dataframe(styled_df, use_container_width=True, hide_index=True)

    # ── Bar chart FALSE count per field ─────────────────────────────────
    st.markdown("#### 📊 FALSE Count per Field (OPEN Orders)")
    chart_data = insight_df.set_index("Field")[["FALSE", "TRUE"]]
    st.bar_chart(chart_data)

    # ── FALSE % bar chart ────────────────────────────────────────────────
    st.markdown("#### 📉 FALSE % per Field (OPEN Orders)")
    pct_chart = insight_df.set_index("Field")[["FALSE %"]].sort_values("FALSE %", ascending=False)
    st.bar_chart(pct_chart)

    # ── Detail baris data yang FALSE — per field (opsional / expandable) ─
    st.markdown("#### 🔍 Detail Baris FALSE per Field (OPEN Orders)")
    st.caption("Klik field untuk melihat daftar PO yang bernilai FALSE.")

    # Kolom identitas yang ingin ditampilkan
    ...
    for col in existing_result_cols:
        ...
        cnt        = len(false_rows)
        if cnt == 0:
            continue  # skip field tanpa FALSE

        # Tentukan kolom SAP & Infor yang relevan untuk field ini
        field_pair_map = {
        ...
        with st.expander(f"❌ {label} — {cnt} FALSE", expanded=False):
            ...
            # Download per-field
            csv_bytes = false_rows[show_cols].to_csv(index=False).encode("utf-8")
            st.download_button(
                label=f"⬇️ Download FALSE rows — {label} (CSV)",
                data=csv_bytes,
                ...
                if infor_all.empty:
                    ...
                    if final_df.empty:
                        ...
                        if submitted or "df_view" in st.session_state:
                            ...
                            # ── Comparison Summary (All Statuses) ─────────────────────────
                            st.subheader("📊 Comparison Summary (TRUE vs FALSE) — Semua Status")
                            existing_results = [
                                c for c in [
                                    "Result_Quantity",
                                    "Result_Market PO",
                            ...
                            if existing_results:
                                ...
                                # TOP FALSE — filter hanya OPEN & UNCONFIRMED (existing behavior)
                            ...
                            # ══════════════════════════════════════════════════════════
                            # [NEW] FALSE Insight — OPEN Orders Only
                            # ══════════════════════════════════════════════════════════
